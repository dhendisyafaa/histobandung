...
export default function FinishPage() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const storedData = localStorage.getItem("histobandung-quiz");
      if (storedData) {
  ...
  const sendEmail = async (e) => {
    ...
    try {
      const emailParams = {
        from_name: "Histobandung",
        player_name: `${player.name}`,
        player_email: email,
        message: `Selamat, kamu telah menyelesaikan quiz di Histobandung. Skormu adalah ${player.score}`,
      };
  ...
  const handleOnPlayAgain = () => {
    ...
    localStorage.setItem("histobandung-quiz", JSON.stringify(resetPlayer));
    push("/quiz");
  };
